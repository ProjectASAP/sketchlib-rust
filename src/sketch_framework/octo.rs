...
//! compact delta entries via SPSC queues when counters overflow a promotion threshold.
//! An aggregator thread applies deltas to a full-precision parent sketch continuously.
...
use crate::{
    CountChild, CountDelta, CountMin, CountMinChild, CmDelta, Count, HllChild, HllDelta,
    HyperLogLog, Regular, RegularPath, SketchInput, Vector2D, hash64_seeded,
};

/// Default SPSC ring buffer capacity per worker.
...
pub struct OctoConfig {
    ...
    pub pin_cores: bool,
    /// SPSC ring buffer capacity per worker (default: 65536).
...
///
/// 1. Partitions `inputs` across `config.num_workers` workers by hash.
/// 2. Each worker maintains a child sketch, emitting deltas via SPSC queues.
/// 3. The aggregator drains all queues continuously, applying deltas to the parent.
    ...
    P: OctoParent<Delta = W::Delta>,
{
    ...
    let capacity = config.queue_capacity.max(1024);

    // Step 1: Partition inputs by hash into per-worker index lists.
    ...
    // Step 2: Create SPSC ring buffers.
    let mut producers = Vec::with_capacity(num_workers);
    let mut consumers = Vec::with_capacity(num_workers);
    for _ in 0..num_workers {
        let rb = HeapRb::<W::Delta>::new(capacity);
        let (prod, cons) = rb.split();
        producers.push(prod);
        consumers.push(cons);
    }

    // Step 3: Finish flags (one per worker).
    let finished: Vec<AtomicBool> = (0..num_workers).map(|_| AtomicBool::new(false)).collect();

    // Step 4: Run workers + aggregator inside a scoped thread block.
    ...
    thread::scope(|s| {
        ...
        for (worker_id, (partition, producer)) in partitions
            .iter()
            .zip(producers.into_iter())
            .enumerate()
        {
            let finished_flag = &finished[worker_id];
            ...
            s.spawn(move || {
                ...
                for &input_idx in partition {
                    let input = &inputs[input_idx];
                    worker.process(input, &mut |delta| {
                        // Spin-wait if the ring buffer is full (backpressure).
                        while prod.try_push(delta).is_err() {
                            std::hint::spin_loop();
                        }
                    });
                }
        ...
        // Aggregator collect loop: drain all consumer queues.
        let all_done = || finished.iter().all(|f| f.load(Ordering::Acquire));

        while !all_done() {
            for cons in consumers.iter_mut() {
                while let Some(delta) = cons.try_pop() {
                    parent.apply(delta);
                }
            }
        }

        // Final drain pass after all workers have finished.
        for cons in consumers.iter_mut() {
            while let Some(delta) = cons.try_pop() {
                parent.apply(delta);
            }
